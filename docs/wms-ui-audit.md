...
| EmptyState | Meaningless empty circle was used. | Empty states looked unfinished and did not explain the missing work. | Icon-based empty state with title, description, action, variants, and WMS-specific icon presets. | Continue applying page-specific icons where pages still use the default. | P1 | `src/components/EmptyState.tsx` | IMPLEMENTED foundation |
...
| `Склады` / `Склады и ячейки` | Warehouse form now uses React Query, Hook Form, Zod, Sonner, DataTable, and action menus; location setup is still denser/manual. | Admin setup with clean forms, readable tables, restrained actions. | Migrate location forms to shared schemas and decompose setup sections. | P0 | `locations/page.tsx`, `warehouses/page.tsx` | PARTIAL |
| `Товары` | Uses React Query, Hook Form, Zod, Sonner, DataTable, and CSV/XLSX preview; still admin-heavy. | Clean catalog admin screen with polished import errors. | Decompose import/create/variant sections and add deeper import E2E. | P1 | `products/page.tsx` | PARTIAL |
